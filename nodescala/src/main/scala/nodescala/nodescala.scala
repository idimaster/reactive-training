...
trait NodeScala {
  ...
  private def respond(exchange: Exchange, token: CancellationToken, response: Response): Unit = {
    response takeWhile {_ => token.nonCancelled} foreach { s=>exchange.write(s)}
    exchange.close()
  }
  ...
  def start(relativePath: String)(handler: Request => Response): Subscription = {
    ...
    Subscription.apply(listenerSubscription, Future.run(){ ct =>
      async {
        while (ct.nonCancelled) {
          val r = await(listener.nextRequest())
          Future.apply(respond(r._2, ct, handler(r._1))).onComplete {
