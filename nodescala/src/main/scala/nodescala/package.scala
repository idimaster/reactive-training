...
package object nodescala {
  ...
  implicit class FutureCompanionOps(val f: Future.type) extends AnyVal {
    ...
    def all[T](fs: List[Future[T]]): Future[List[T]] = {
      fs match {
        case Nil => Future(Nil)
        case (f::fts) => f.flatMap(t => all[T](fts).flatMap(ts => Future[List[T]](t::ts)))
      }
    }
    ...
    def any[T](fs: List[Future[T]]): Future[T] = {
      val r = Promise[T]()
      fs.foreach((f:Future[T])=> f.onComplete {
        case Failure(e) => r.tryFailure(e)
        case Success(x) => r.trySuccess(x)
    })
      r.future
    }
    ...
    def run()(f: CancellationToken => Future[Unit]): Subscription = {
      ...
      f(ct.cancellationToken).onComplete {
  ...
  implicit class FutureOps[T](val f: Future[T]) extends AnyVal {
    ...
    def continueWith[S](cont: Future[T] => S): Future[S] = {
      val r = Promise[S]()
      f.onComplete {
        case Failure(e) => r.failure(e)
        case Success(x) => {
          try {
            r.success(cont(f))
          }
          catch {
            case t: Exception => r.failure(t)
          }
        }
      }
      r.future
      ...
      /*
    def continueWith2[S](cont: Future[T] => S): Future[S] = async {
      await(f)
      cont(f)
    }  */
     ...
     *  The resulting future contains a value returned by `cont`.
     */
  /*  def continue2[S](cont: Try[T] => S): Future[S] = async {
      await(f)
      cont(f.value.get)
    } */
